...
/*   Updated: 2024/06/05 10:50:21 by gmersch          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*apply_var_env(char **arg, int i, t_minishell *mini, t_pars *p)
{
	...
	while (arg[i][j])
	{
		...
		//printf("%d = ddonquote\n", on_quote[1]);
		//printf("%c = c\n", arg[i][j]);
		if (arg[i][j] == '$' && (ft_isalnum(arg[i][j + 1]) || arg[i][j + 1] == '?' || ((arg[i][j + 1] == '\"' || arg[i][j + 1] == '\'') && (!on_quote[0] && !on_quote[1]))) && !on_quote[0] && p->next_is_hd_stop == false)
		{
			//printf("JESUISDEDANS\n");
			p->is_expand[i] = true;
